...
model_path = os.path.join(base_dir, 'mlp_model_romaniandeadlift.pkl')
label_encoder_path = os.path.join(base_dir, 'label_encoder_romaniandeadlift.pkl')
    ...
    while cap.isOpened():
        if not paused:
            ...
            try:
                ...
                # Points for Romanian Deadlift posture
                ...
                print(f"Knee Angle: {knee_angle}")  # Debugging

                # Display the angle
                cv2.putText(image, f"Knee Angle: {int(knee_angle)}",
                            (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)

                # Romanian deadlift counting logic
                if knee_angle > 160 and stage != "up":  # "up" position
                    stage = "up"
                    print(f"Stage set to 'up'. Angle: {knee_angle}")  # Debugging

                if knee_angle < 140 and stage == "up":  # "down" position
                    counter += 1  # Increment counter when repetition is complete
                    stage = "down"
                    print(f"Repetition counted: {counter}. Stage set to 'down'. Angle: {knee_angle}")  # Debugging
