...
# # Define the path to the 'scripts/russian_twist' directory
# base_dir = os.path.dirname(os.path.abspath(__file__))  # Current script directory
# model_path = os.path.join(base_dir, 'mlp_model_russiantwist.pkl')
# label_encoder_path = os.path.join(base_dir, 'label_encoder_russiantwist.pkl')
...
# else:
#     raise FileNotFoundError("One or both .pkl files are missing in the 'scripts/russian_twist' directory.")
...
# # Counter and stage variables for Russian Twist
# counter = 0
# stage = None
...
#             # Points for Russian Twist posture
...
#             # Calculate the torso twist angle
#             twist_angle = calculate_angle(left_shoulder, left_hip, left_knee)

#             # Display the angle
#             cv2.putText(image, f"Twist Angle: {int(twist_angle)}", 
#                         tuple(np.multiply(left_hip, [640, 480]).astype(int)), 
#                         cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)

#             # Russian Twist counting logic based on angle
#             if twist_angle > 120:
#                 stage = "center"
#             if twist_angle < 80 and stage == "center":
#                 stage = "twist"
#                 counter += 1
#                 print(f"Reps: {counter}")

#             # Prepare feature vector for model prediction
#             input_features = np.array([twist_angle]).reshape(1, -1)
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            # Calculate the torso twist angle
            twist_angle = calculate_angle(left_shoulder, left_hip, left_knee)
            print(f"Twist Angle: {twist_angle}")  # Debugging

            # Display the angle
            cv2.putText(image, f"Twist Angle: {int(twist_angle)}",
                        (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)

            # Russian Twist counting logic
            if twist_angle > 100 and stage != "up":  # Adjusted threshold for "up" position
                stage = "up"
                print(f"Stage set to 'up'. Angle: {twist_angle}")  # Debugging

            if twist_angle < 70 and stage == "up":  # Adjusted threshold for "down" position
                counter += 1  # Increment counter when twist is complete
                stage = "down"
                print(f"Repetition counted: {counter}. Stage set to 'down'. Angle: {twist_angle}")  # Debugging
            ...
            # Display the feedback
            cv2.putText(image, f"Feedback: {predicted_feedback}",
                        (30, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
