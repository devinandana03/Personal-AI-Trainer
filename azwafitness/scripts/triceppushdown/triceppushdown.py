...
# # Define the path to the 'scripts/tricep_pushdowns' directory
# base_dir = os.path.dirname(os.path.abspath(__file__))  # Current script directory
# model_path = os.path.join(base_dir, 'mlp_model_triceppushdown.pkl')
# label_encoder_path = os.path.join(base_dir, 'label_encoder_triceppushdown.pkl')
...
# else:
#     raise FileNotFoundError("One or both .pkl files are missing in the 'scripts/tricep_pushdowns' directory.")
...
# # Counter and stage variables for tricep pushdowns
# counter = 0
# stage = None
...
#             # Points for tricep pushdown posture
...
#             # Tricep pushdown counting logic based on elbow angle
#             if elbow_angle > 160:
#                 stage = "up"
    ...
    while cap.isOpened():
        if not paused:
            ...
            try:
                ...
                # Tricep pushdown counting logic
                if elbow_angle > 110 and stage != "up":  # "up" position
                    stage = "up"
                    print(f"Stage set to 'up'. Angle: {elbow_angle}")  # Debugging

                if elbow_angle < 70 and stage == "up":  # "down" position
                    counter += 1  # Increment counter when pushdown is complete
                    stage = "down"
